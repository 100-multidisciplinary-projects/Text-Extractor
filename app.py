...
Upload images via file path and get OCR results with AI validation
"""
...
def process_image(image_path, output_dir, ocr_engine, preprocessing_mode, 
                 api_key, enable_ai, show_details, ocr_tamil, paddle_ocr):
    ...
    if preprocessing_mode.lower() == "auto":
        ...
        if original_text:
            ...
            # Display metrics
            print(f"\n{Colors.BOLD}OCR Results (Original Preprocessing):{Colors.ENDC}")
            ...
            with open(result_file, 'w', encoding='utf-8') as f:
                ...
                f.write(f"Preprocessing: Original\n")
            ...
            print_error("Could not recognize any text with any method")
        ...
        if raw_text:
            ...
            print_error("Could not recognize any text. Try 'auto' mode.")


# ---------- Main Function ----------
def main():
    parser = argparse.ArgumentParser(
        description="Tamil OCR Terminal Application with AI Validation",
...
Examples:
  ...
  python tamil_ocr_terminal.py img1.jpg img2.jpg --output ./results
        """
    )
    ...
    parser.add_argument('--api-key', '-k', default='', 
                       help='OpenRouter API key for AI validation')
    ...
    if args.ai and not args.api_key:
        print_warning("AI validation enabled but no API key provided!")
        print_info("Use --api-key YOUR_KEY to enable AI validation")
        args.ai = False
